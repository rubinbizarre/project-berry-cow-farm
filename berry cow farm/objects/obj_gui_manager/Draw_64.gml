...
if (obj_farm_manager.milk_chart_active) {
	if (surface_exists(obj_farm_manager.milk_surface)) and (instance_exists(obj_window_milk)) {
		draw_surface(obj_farm_manager.milk_surface, obj_window_milk.gui_x + 40, obj_window_milk.gui_y + 180);
	}
}
#endregion
...
switch (room) {
	...
	case rm_main: {
		...
		if (global.debug) {
			...
			draw_text(20, 80, "camera_x: "+string(camera_get_view_x(obj_master.camera)));
		...
		var gui_scale_x = view_get_wport(0) / obj_master.base_viewport_width;
		var gui_scale_y = view_get_hport(0) / obj_master.base_viewport_height;
